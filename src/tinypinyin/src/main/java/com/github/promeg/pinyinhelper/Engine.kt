 ...
 */
object Engine {
    private val EMIT_COMPARATOR = EmitComparator()
    ...
    fun toPinyin(
        ...
        while (i < inputStr.length) {
            ...
            if (nextHitIndex < selectedEmits.size && i == selectedEmits[nextHitIndex].start) {
                // 有以第i个字符作为begin的hit
                val fromDicts =
                    pinyinFromDict(selectedEmits[nextHitIndex].keyword, pinyinDictList)!!
    ...
    fun select(
        inputStr: String, trie: Trie, selector: SegmentationSelector
    ): List<Emit>? {
        val selectedEmits = selector.select(trie.parseText(inputStr))!!
        Collections.sort(selectedEmits, EMIT_COMPARATOR)
